...
""",
    ...
    def test_compact_result_keeps_required_readback_without_full_lists(self) -> None:
        module = load_selector_module()
...
""",
            ...
            result = module.select_task(root)
            compact = module.compact_selector_result(result)

        self.assertEqual(compact["status"], "no_ready")
        self.assertIsNone(compact["selected"])
        self.assertEqual(compact["summary"]["blocking_in_progress_count"], 1)
        self.assertEqual(compact["blocking_in_progress"][0]["id"], "VT-001")
        self.assertEqual(compact["dependency_pending_preview"][0]["id"], "VT-002")
        self.assertNotIn("ready", compact)
        self.assertNotIn("dependency_pending", compact)
...
""",
    ...
    def test_cli_compact_json_resets_expired_in_progress(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            backlog = root / "backlog"
            backlog.mkdir()
...
""",
            ...
            completed = subprocess.run(
                [sys.executable, str(SCRIPT_PATH), "--root", str(root), "--compact-json"],
                capture_output=True,
                check=False,
                text=True,
                timeout=5,
