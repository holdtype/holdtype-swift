...
def compact_task_json(task_json: dict[str, Any]) -> dict[str, Any]:
    compact = {
        key: task_json[key]
        for key in ("id", "path", "title", "status", "priority", "lane")
        if key in task_json
    }
    for key in (
        "dependencies",
        "unmet_dependencies",
        "unmet_dependency_statuses",
        "blocked_candidates",
        "unblocks",
    ):
    ...
    for key in (
        ...
        if key in task_json:
            compact[key] = task_json[key]
    return compact


def compact_selector_result(result: dict[str, Any]) -> dict[str, Any]:
    compact: dict[str, Any] = {
        "status": result["status"],
        "selected": (
            compact_task_json(result["selected"])
    ...
    for key in (
        "selection_reason",
        "errors",
        "expired_in_progress_applied",
        "expired_in_progress_reset_paths",
    ):
        if key in result:
            compact[key] = result[key]

    for key in ("in_progress", "blocking_in_progress", "expired_in_progress"):
        items = result.get(key)
        if items:
            compact[key] = [compact_task_json(item) for item in items]

    dependency_pending = result.get("dependency_pending")
    if dependency_pending:
        compact["dependency_pending_preview"] = [
            compact_task_json(item) for item in dependency_pending[:3]
        ]

    deferred = result.get("deferred")
    if deferred:
        compact["deferred_preview"] = [compact_task_json(item) for item in deferred[:3]]
...
def main() -> int:
    ...
    parser.add_argument(
        "--compact-json",
        action="store_true",
        help="print compact machine-readable JSON for agent readback",
    )
    parser.add_argument(
    ...
    if args.json and args.compact_json:
        parser.error("--json and --compact-json are mutually exclusive")
    ...
    if args.compact_json:
        print(json.dumps(compact_selector_result(result), indent=2, sort_keys=True))
    elif args.json:
