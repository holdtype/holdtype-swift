...
struct IOSContainingAppRecoveryTests {
    @Test func capturedForegroundAcceptanceUsesOneProductionRootLease()
        async throws {
        ...
        #expect(registry.context(for: root) === context)

        let sourceURL = root.appendingPathComponent(
            "captured-foreground-source-\(UUID().uuidString).wav"
        )
        ...
        var dispatch: IOSForegroundVoiceTranscriptionDispatch? =
            try await owner.beginTranscription(
                expected: IOSPendingRecordingCASExpectation(
                    recording: prepared
                ),
                transcriptionID: UUID()
            )
        let transcribing = try #require(dispatch?.recording)
        ...
        let transcriptID = try #require(outputDelivery.transcriptionID)
        let preparation = try IOSForegroundVoiceAcceptedOutputPreparation(
            deliveryID: UUID(),
            sessionID: UUID(),
            attemptID: outputDelivery.attemptID,
            transcriptID: transcriptID,
            rawAcceptedText: "Captured production acceptance",
            outputIntent: outputDelivery.outputIntent,
            keepLatestResult: true,
            historyMode: .captured(capture)
        )

        // The owner already holds the context's root gate here. A public
        // coordinator re-entry would throw instead of reaching this terminal
        // result, so success proves the real coordinator consumed that lease.
        ...
        guard case .resultReady(let accepted) = result else {
            Issue.record("Expected terminal captured foreground acceptance")
            return
        }
        #expect(accepted.historyWrite?.state == .committed)
        #expect(
        ...
        #expect(!FileManager.default.fileExists(atPath: audioURL.path))
        #expect(
            try await owner.loadLatestResult() == .resultReady(accepted)
        )

        let historyBeforeRecovery = try #require(
            try FoundationIOSAcceptedHistoryJournalRepository(
                applicationSupportDirectoryURL: root
            ).load()?.envelope
        )
        let row = try #require(historyBeforeRecovery.entries.first)
        #expect(historyBeforeRecovery.entries.count == 1)
        #expect(row.deliveryID == accepted.deliveryID)
        #expect(row.transcriptID == accepted.transcriptID)
        #expect(row.acceptedText == accepted.acceptedText)
        #expect(
            try FoundationIOSAcceptedHistoryOutboxJournalRepository(
                applicationSupportDirectoryURL: root
            ).load()?.envelope.entries.isEmpty != false
        )
        ...
        #expect(
            try FoundationIOSAcceptedHistoryJournalRepository(
                applicationSupportDirectoryURL: root
            ).load()?.envelope == historyBeforeRecovery
        )
        #expect(
            try FoundationIOSAcceptedHistoryOutboxJournalRepository(
                applicationSupportDirectoryURL: root
