...
/// Grants one whole credential transaction at a time and shields an acquired
/// transaction from ordinary caller cancellation until it finalizes.
actor CredentialOperationGate {
    ...
    private final class Waiter: @unchecked Sendable {
        ...
        func install(
            ...
            case .granted:
                lock.unlock()
                assertionFailure("A credential waiter cannot be granted before installation.")
                continuation.resume(throwing: AcquisitionError.cancelledBeforeLease)
                return false
            }
        }
    ...
    func perform<Value: Sendable>(
        ...
        let operationTask = Self.startCancellationShieldedTask(operation)
    ...
    private func release(_ lease: Lease) {
        guard activeLease == lease else {
            assertionFailure("Only the active credential-operation lease may be released.")
            return
        }
    ...
    private nonisolated static func startCancellationShieldedTask<Value: Sendable>(
        _ operation: @escaping @Sendable () async throws -> Value
    ) -> Task<Value, Error> {
        Task {
            try await operation()
