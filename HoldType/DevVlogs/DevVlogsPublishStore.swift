...
final class DevVlogsPublishStore: ObservableObject {
    ...
    private func selectDay(_ day: DevVlogsLibraryDay) {
        ...
        orderedClipIDs = day.clips.compactMap(\.clipID)
        selectedClipIDs = Set(
            day.clips.filter { $0.isBuildEligible && !$0.isExcluded }.compactMap(\.clipID)
        )
        recipe = nil
        workspace = nil
        updateReadyPresentation()
    ...
    private func appendNewClips(from day: DevVlogsLibraryDay) {
        ...
        for clip in day.clips where clip.clipID.map({ !existing.contains($0) }) == true {
    ...
    private func runRetry() async {
        ...
        do {
            let access = try destinationAccessProvider()
            defer { access.release() }
            if FileManager.default.fileExists(atPath: workspace.finalOutputURL.path) {
                let existing = try await mediaBuilder.validateOutput(at: workspace.finalOutputURL)
            ...
            await recipeRepository.removeTemporaryOutput(workspace: workspace)
    ...
    private func render(
        ...
        let sources = try recipe.orderedClipIDs.map { clipID -> DevVlogsBuildSource in
            guard let clip = clipsByID[clipID],
                  clip.health == .ready,
                  let mediaURL = clip.mediaURL else {
                throw DevVlogsBuildError.sourceMissing
            }
            return DevVlogsBuildSource(clipID: clipID, fileURL: mediaURL)
        }
    ...
    private func finishFailure(_ error: Error) async {
        ...
        presentation = DevVlogsPublishPresentation(
            state: state,
            enabledActions: recipe == nil || workspace == nil ? [] : [.retry]
        )
    }
    ...
    private func orderedClips(in day: DevVlogsLibraryDay) -> [DevVlogsLibraryClip] {
        let byID = Dictionary(uniqueKeysWithValues: day.clips.map { ($0.id, $0) })
        let ordered = orderedClipIDs.compactMap { byID[$0.uuidString.lowercased()] }
        let remaining = day.clips.filter { !ordered.map(\.id).contains($0.id) }
