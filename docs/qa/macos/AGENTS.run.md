...
Use this guide for bounded Computer Use QA on the HoldType macOS app.
...
- Launch automation with live Keychain access disabled and a sanitized app
  environment, normally via `script/build_and_run.sh --verify`. Do not enter
  the macOS login keychain password or click `Always Allow` during Computer Use
  QA.
...
1. Build and launch the app through the repository automation path, usually
   `script/build_and_run.sh --verify`, unless the task requires a more specific
   `xcodebuild` command.
2. Stop or replace only run-owned app instances when ownership is clear.
3. Confirm the freshly built app is the launched app.
