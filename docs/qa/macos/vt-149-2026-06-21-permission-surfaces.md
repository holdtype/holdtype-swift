...
Build/Test:
- `xcodebuild -project holdtype.xcodeproj -scheme holdtype -destination 'platform=macOS' build` passed.
- `xcodebuild -project holdtype.xcodeproj -scheme holdtype -destination 'platform=macOS' build-for-testing -only-testing:holdtypeTests/PermissionsServiceTests` passed.
- `xcodebuild -project holdtype.xcodeproj -scheme holdtype -destination 'platform=macOS' test -only-testing:holdtypeTests/PermissionsServiceTests` failed before assertions with `IDELaunchServicesLauncher - Failed to Launch`.
Runtime QA: blocked
