...
Build/Test: `xcodebuild -project holdtype.xcodeproj -scheme holdtype -destination 'platform=macOS' -derivedDataPath /tmp/holdtype-swift-vt112-deriveddata build`
Runtime QA: blocked
...
2. Built the macOS app into run-owned DerivedData at
   `/tmp/holdtype-swift-vt112-deriveddata`.
3. Launched the freshly built app executable from
   `/tmp/holdtype-swift-vt112-deriveddata/Build/Products/Debug/holdtype.app`.
...
- App log: `/tmp/holdtype-swift-vt112-app.log` was empty.
