...
| Script checks | Shell syntax, help/default-help, invalid/extra/mutually-exclusive argument rejection, bounded build-only execution, timeout-wrapped build-settings inspection, and planned-duration-plus-300-second hardware supervision passed. Camera-request supervision uses one absolute monotonic 420-second post-launch deadline; identity and terminal probes, sleeps, natural exit, TERM/KILL revalidation and waits, reap, and trap cleanup are each capped to its remaining time. Permission-mode fakes proved natural exit/reap, stuck-terminal exact TERM/KILL failure, identity-mismatch no-signal failure, trap cleanup, and a slow identity probe that failed without signaling in 2.041 seconds—inside its 2.25-second wall-test bound—under an injected 2-second supervisor deadline. Test-owned mismatch roots/processes were cleaned exactly after assertions. Neither real hardware nor permission-request mode was run. |
