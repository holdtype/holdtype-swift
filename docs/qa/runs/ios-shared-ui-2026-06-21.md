...
- XcodeBuildMCP `build_run_sim` with scheme `holdtype-iOS`
  - Result: timed out after 300 seconds before producing a screenshot.
- `/opt/homebrew/bin/timeout 180 xcodebuild -project holdtype.xcodeproj -scheme holdtype -destination 'platform=macOS' build`
  - Result: timed out with `BUILD INTERRUPTED`; no compiler diagnostics were
    emitted before timeout.
- `/opt/homebrew/bin/timeout 180 xcodebuild -project holdtype.xcodeproj -scheme holdtype-iOS -destination 'platform=iOS Simulator,id=6ACF3054-A7EA-4182-8D0D-996004730391' build`
  - Result: timed out with `BUILD INTERRUPTED`; no compiler diagnostics were
    emitted before timeout.
- `/opt/homebrew/bin/timeout 60 zsh -lc 'SDK=$(xcrun --sdk iphonesimulator --show-sdk-path); xcrun swiftc -typecheck -target arm64-apple-ios17.0-simulator -sdk "$SDK" Shared/HoldTypeSetupStatusView.swift holdtypeIOS/HoldTypeIOSApp.swift'`
  - Result: passed.
- `/opt/homebrew/bin/timeout 60 zsh -lc 'SDK=$(xcrun --sdk macosx --show-sdk-path); xcrun swiftc -typecheck -target arm64-apple-macos26.5 -sdk "$SDK" Shared/HoldTypeSetupStatusView.swift holdtype/*.swift holdtype/Models/*.swift holdtype/Services/*.swift'`
  - Result: passed.
...
it succeeds, rerun XcodeBuildMCP `build_run_sim` for scheme `holdtype-iOS` and
