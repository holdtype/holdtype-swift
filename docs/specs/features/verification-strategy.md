...
Define how HoldType proves MVP dictation behavior without depending on live
...
- UI tests and automated runtime smoke must launch HoldType with the repository
  automation policy, which avoids live Keychain access entirely. A macOS login
  keychain authentication dialog during automated verification is a regression
  or launch-path blocker, not an operator chore.
- Explicit Debug key-file launches are manual live-debug tooling only. Normal
  XCTest, UI test, Computer Use, and repository runtime QA must ignore that
  source and must not call live OpenAI.
- Tests must prove the API key is not stored in UserDefaults and is not logged.

### Text output and Last Result

- Use fake app-clipboard and text-insertion clients for output decision tests.
- Cover Last Result save enabled, disabled behavior, missing
  Accessibility, empty transcript suppression, insertion failure, and insertion
  timeout.
- Real active-app Paste Last Result behavior is bounded runtime smoke or
