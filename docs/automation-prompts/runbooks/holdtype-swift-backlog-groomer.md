...
automationRole: holdtype-swift-backlog-groomer
status: active
---

# HoldType Swift Backlog Groomer Runbook

This runbook is the versioned runtime contract for the current user's
`holdtype-swift-backlog-groomer` installed Codex automation.
...
finishing with the final report only. This automation must not call
`set_thread_archived`; a separate housekeeping sweeper may archive the
completed run later by explicit `threadId` after readback verification. Clean
only current-run temporary artifacts that are not durable evidence. For stale
local Xcode/build/test
...
resources with reasons, `Thread archive: external_sweeper`,
UI/functionality coverage rows updated or explicitly unchanged, unrelated dirty
files preserved, and any blocker.
