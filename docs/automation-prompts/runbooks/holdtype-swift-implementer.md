...
automationRole: holdtype-swift-implementer
status: active
---

# HoldType Swift Implementer Runbook

This runbook is the versioned runtime contract for the current user's
`holdtype-swift-implementer` installed Codex automation.
...
Work on the HoldType Swift development backlog as one bounded 10-minute
iteration.
...
Use full `--json` only when compact output lacks diagnostics needed for a
specific queue decision.
...
residual resource that cannot be terminated, and finish with the final report
only. This automation must not call `set_thread_archived`; a separate
housekeeping sweeper may archive the completed run later by explicit
`threadId` after readback verification. Stale local Xcode/build/test/simulator
...
Use path used, and a short `Product delta` field. Include
`Thread archive: external_sweeper`. The report must also include a `Runtime
QA` field with one of `required`,
