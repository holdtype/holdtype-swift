...
automationRole: holdtype-swift-blocker-resolver
status: active
---

# HoldType Swift Blocker Resolver Runbook

This runbook is the versioned runtime contract for the current user's
`holdtype-swift-blocker-resolver` installed Codex automation.
...
- `docs/automation-prompts/runbooks/holdtype-swift-implementer.md`
...
4. Run `python3 scripts/backlog_next.py --compact-json` before blocked
   selection. This normal selector expires stale implementation claims by
   default while keeping readback small.
5. If `expired_in_progress_reset_paths` is non-empty, run `git diff --check`,
   ...
   commit, and rerun `python3 scripts/backlog_next.py --compact-json`.
...
terminated, and finish with the final report only. This automation must not
call `set_thread_archived`; a separate housekeeping sweeper may archive the
completed run later by explicit `threadId` after readback verification. For
...
resources and any
residual resources with reasons, `Thread archive: external_sweeper`,
completion commit hashes if files changed, final blocked selector result,
actual cwd, execution environment, unrelated dirty files preserved, and
confirmation that the canonical checkout now contains all status or
resolution-path updates.
