...
automationRole: holdtype-swift-backlog-archiver
status: active
---

# HoldType Swift Backlog Archiver Runbook

This runbook is the versioned runtime contract for the current user's
`holdtype-swift-backlog-archiver` installed Codex automation.
...
Run one bounded completed-backlog archival pass for HoldType Swift.
...
artifacts, report any residual resource that cannot be terminated, and finish
with the final report only. This automation must not call
`python3 scripts/automation_resource_cleanup.py`, and it must not call
`set_thread_archived`; a separate housekeeping sweeper may archive the
completed run later by explicit `threadId` after readback verification.
