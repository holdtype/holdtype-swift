...
struct IOSForegroundVoiceWorkflowTests {
    ...
    @Test
    func keyboardHandoffPreflightReusesVoiceGatesWithoutStartingCapture()
        async throws {
        let fixture = try await WorkflowFixture(
            permission: .granted,
            preacceptConsent: true
        ...
        let result = await fixture.workflow.preflightKeyboardHandoff(
            action: .standard,
            sceneLease: fixture.lease
        )
        ...
        #expect(fixture.events.contains("credential-resolve"))
        #expect(fixture.events.contains("permission-read"))
        #expect(!fixture.events.contains("history-stop"))
        #expect(!fixture.events.contains("audio-activate"))
        #expect(!fixture.events.contains("recording-make"))
        #expect(!fixture.events.contains("provider-process"))
    }
    ...
    @Test
    func keyboardHandoffPreflightRoutesEveryExistingSetupOwnerBeforeCapture()
        async throws {
        var invalidTranscription = IOSAppSettings.defaults
        invalidTranscription.transcriptionConfiguration =
            TranscriptionConfiguration(
                language: .custom,
                customLanguageCode: "invalid!"
            )
        let transcription = try await WorkflowFixture(
            settings: invalidTranscription,
            permission: .granted
        )
        #expect(
            await transcription.workflow.preflightKeyboardHandoff(
                action: .standard,
                sceneLease: transcription.lease
            ) == .needsSetup(.transcription, failure: .unavailable)
        )

        let translation = try await WorkflowFixture(
            permission: .granted
        )
        #expect(
            await translation.workflow.preflightKeyboardHandoff(
                action: .translate,
                sceneLease: translation.lease
            ) == .needsSetup(.translation, failure: .unavailable)
        )

        let consent = try await WorkflowFixture(permission: .granted)
        #expect(
            await consent.workflow.preflightKeyboardHandoff(
                action: .standard,
                sceneLease: consent.lease
            ) == .needsSetup(.microphoneAndPrivacy, failure: nil)
        )
        #expect(!consent.events.contains("consent-continue"))
        ...
        #expect(
            await credential.workflow.preflightKeyboardHandoff(
                action: .standard,
                sceneLease: credential.lease
            ) == .needsSetup(.openAI, failure: nil)
        )

        let denied = try await WorkflowFixture(
            permission: .denied,
            preacceptConsent: true
        )
        #expect(
            await denied.workflow.preflightKeyboardHandoff(
                action: .standard,
                sceneLease: denied.lease
            ) == .needsSetup(
                .microphoneAndPrivacy,
    ...
    @Test
    func keyboardHandoffPreflightRequestsUndeterminedMicrophoneInline()
        async throws {
        let fixture = try await WorkflowFixture(
            permission: .undetermined,
        ...
        #expect(result == .ready)
        #expect(fixture.permissionRequestCount == 1)
        #expect(fixture.events.count("permission-request") == 1)
        #expect(!fixture.events.contains("history-stop"))
        #expect(!fixture.events.contains("audio-activate"))
        #expect(!fixture.events.contains("recording-make"))
    }
