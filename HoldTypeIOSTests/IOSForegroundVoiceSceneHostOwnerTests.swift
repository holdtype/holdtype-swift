...
struct IOSForegroundVoiceSceneHostOwnerTests {
    ...
    @Test
    func keyboardHandoffPreflightRequiresAndUsesThisExactActiveScene()
        async throws {
        let fixture = VoiceSceneHostClientFixture()
        let preflight = VoiceSceneHostPreflightFixture()
        let registry = IOSVoiceSceneRegistry()
        let controller = IOSForegroundVoiceController(
            client: fixture.makeClient(),
            sceneRegistry: registry
        ...
        let owner = IOSForegroundVoiceSceneHostOwner(
            controller: controller,
            sceneRegistry: registry,
            preflightKeyboardHandoff: { action, lease in
                preflight.run(
                    action: action,
                    lease: lease,
                    registry: registry
        ...
        let issuedAt = Date()
        let possibleIntent: HoldTypeIOS.KeyboardHandoffIntentRecord? = .init(
            requestID: UUID(),
            sourceDocumentID: UUID(),
            action: HoldTypeIOS.KeyboardVoiceAction.translateAndImprove,
            issuedAt: issuedAt,
            expiresAt: issuedAt.addingTimeInterval(5)
        )
        guard let intent = possibleIntent else {
            Issue.record("Expected a valid handoff intent fixture.")
            return
        }

        #expect(
            await owner.preflightKeyboardHandoff(intent)
                == IOSKeyboardHandoffPreflightResult.unavailable(.unavailable)
        )
        #expect(preflight.callCount == 0)

        #expect(
            owner.register(initialActivity: IOSVoiceSceneActivity.active)
                == .accepted
        )
        #expect(await owner.preflightKeyboardHandoff(intent) == .ready)
        #expect(preflight.callCount == 1)
        #expect(
            preflight.action
                == HoldTypeIOS.KeyboardVoiceAction.translateAndImprove
        )
        #expect(preflight.leaseWasReady)

        #expect(
            owner.updateActivity(IOSVoiceSceneActivity.background)
                == .accepted
        )
        #expect(
            await owner.preflightKeyboardHandoff(intent)
...
@MainActor
private final class VoiceSceneHostPreflightFixture {
    private(set) var callCount = 0
    private(set) var action: HoldTypeIOS.KeyboardVoiceAction?
    private(set) var leaseWasReady = false

    func run(
        action: HoldTypeIOS.KeyboardVoiceAction,
        lease: IOSVoiceSceneStartLease,
        registry: IOSVoiceSceneRegistry
    ) -> IOSKeyboardHandoffPreflightResult {
        callCount += 1
        self.action = action
        leaseWasReady = registry.validateContinuation(lease) == .ready
        return .ready
    }
}
