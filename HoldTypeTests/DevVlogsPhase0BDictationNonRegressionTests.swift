...
struct DevVlogsPhase0BDictationNonRegressionTests {
    ...
    @Test func cameraSlowTimeoutPairMatchesBaseline() async throws { if #available(macOS 15.0, *) { try await DevVlogsPhase0BE07PairHarness().assertPair(caseID: .camera_slow_timeout) } }
    ...
    @Test func evidenceArtifactsMatchClosedSchemasAndRedactionPolicy() throws { try DevVlogsPhase0BE07EvidenceValidator().validate(fromTestFilePath: #filePath) }
}
#endif
