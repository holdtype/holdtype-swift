...
struct DictationSessionControllerRecordingActionTests {
    ...
    @Test func overlappingStopRequestsUseOneTranscription() async {
        ...
        let artifact = AudioRecordingArtifact(
            fileURL: URL(fileURLWithPath: "/tmp/holdtype-recording-action-stop.m4a"),
            duration: 1.1,
            byteCount: 4096
        )
        let recorder = RecordingActionRecorder(
...
private struct RecordingActionStopTailSleeper: RecordingStopTailSleeping {
    func sleep(seconds: TimeInterval) async throws {}
}

private struct RecordingActionCredentialResolver: OpenAICredentialResolving {
    func resolveOpenAICredential() throws -> OpenAICredential {
        try OpenAICredential(apiKey: "sk-recording-action-test")
    }
}
