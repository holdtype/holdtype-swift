...
struct SpecialClipboardHotkeyServiceTests {
    ...
    @Test func coordinatorRegistersShortcutWhenAppClipboardIsEnabled() async throws {
        ...
        #expect(coordinator.lastStatusText == "Inserted last result.")
    }
    ...
    @Test func coordinatorDoesNotRegisterShortcutWhenAppClipboardIsDisabled() async {
        ...
        #expect(coordinator.lastStatusText == "Paste Last Result is disabled.")
    }

    @Test func carbonRegistrationUsesControlCommandVOnRelease() {
        #expect(SpecialClipboardHotkeyCarbonRegistration.keyCode == UInt32(kVK_ANSI_V))
        #expect(SpecialClipboardHotkeyCarbonRegistration.modifiers == UInt32(controlKey | cmdKey))
        #expect(SpecialClipboardHotkeyCarbonRegistration.eventKind == UInt32(kEventHotKeyReleased))
    }
    ...
    private func makeSettingsStore() -> (AppSettingsStore, UserDefaults, String) {
        let suiteName = "holdtype.SpecialClipboardHotkeyServiceTests.\(UUID().uuidString)"
