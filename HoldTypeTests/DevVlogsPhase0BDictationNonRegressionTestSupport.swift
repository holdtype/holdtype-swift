...
struct DevVlogsPhase0BE07EvidenceValidator {
    ...
    private func validateMarkdown(_ run: URL) throws {
        ...
        for (name, headings, claims) in specifications {
            let text = try String(contentsOf: run.appendingPathComponent(name), encoding: .utf8); guard text.split(separator: "\n").filter({ $0.hasPrefix("#") }).map(String.init) == headings, claims.allSatisfy(text.contains) else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidHeading }
            if name == "summary.md" {
                let results = ["pass", "fail"].filter { text.components(separatedBy: "Functional result: \($0).").count == 2 }; guard results.count == 1 else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema }
                let unequalLines = text.split(separator: "\n").map(String.init).filter { $0.hasPrefix("Unequal case:") }; let caseOrder = DevVlogsPhase0BE07CaseID.allCases.map(\.rawValue)
                let namedCases = unequalLines.compactMap { line in caseOrder.first { line == "Unequal case: \($0)." } }; guard namedCases.count == unequalLines.count else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema }
                let unequalCaseField = namedCases.isEmpty ? ["none"] : namedCases
                if results[0] == "pass" { guard unequalCaseField == ["none"] else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema } }
                else { guard !namedCases.isEmpty, namedCases == caseOrder.filter(namedCases.contains), Set(namedCases).count == namedCases.count else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema } }
            }
        }
    }
    ...
    private func validateCSVFiles(_ run: URL) throws {
        ...
        for (index, row) in matrixRows.enumerated() { let caseID = DevVlogsPhase0BE07CaseID.allCases[index]; let cancel = caseID == .explicit_cancel; let observer = ["ready", "ready", "ready", "cancelled", "camera_unavailable", "camera_busy", "camera_preparation_timed_out", "destination_unavailable", "destination_disconnected", "mux_failed"][index]; let access = [0, 1, 2, 9].contains(index) ? 1 : 0; let agreement = (row[4], row[17]); guard row[0] == "dv-p0b-e07-matrix-v1", row[1] == caseID.rawValue, row[2] == (cancel ? "cancelled" : "success"), row[3] == row[2], agreement == ("true", "pass") || agreement == ("false", "fail"), row[5] == observer, row[6] == "1", row[7] == "1", row[8] == (cancel ? "0" : "1"), row[9] == row[8], row[10] == row[8], row[11] == row[8], row[12] == String(access), row[13] == String(access), row[14] == "1", row[15] == "0", row[16] == "0", row[18] == "functional_gate" else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema } }
    ...
    private func validateEvents(_ run: URL) throws {
        ...
        var eventAgreement: [Bool] = []
        for (index, line) in lines.enumerated() { let text = String(line); for key in keys where text.components(separatedBy: "\"\(key)\"").count != 2 { throw DevVlogsPhase0BE07EvidenceValidationError.duplicateKey }; guard let object = try JSONSerialization.jsonObject(with: Data(line.utf8)) as? [String: Any], Set(object.keys) == keys, object["schema_version"] as? String == "dv-p0b-e07-events-v1", object["sequence"] as? Int == index + 1, object["case_id"] as? String == DevVlogsPhase0BE07CaseID.allCases[index / 8].rawValue else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidOrder }; let position = index % 8; let caseID = DevVlogsPhase0BE07CaseID.allCases[index / 8]; let cancel = caseID == .explicit_cancel; let observer = ["ready", "ready", "ready", "cancelled", "camera_unavailable", "camera_busy", "camera_preparation_timed_out", "destination_unavailable", "destination_disconnected", "mux_failed"][index / 8]; let modes = ["baseline", "baseline", "baseline", "spike", "spike", "spike", "spike", "pair"]; let actions = ["attempt_started", "dictation_recording", "dictation_terminal", "attempt_started", "dictation_recording", "observer_terminal", "dictation_terminal", "pair_compared"]; let terminal = cancel ? "cancelled" : "success"; let results = ["started", "recording", terminal, "started", "recording", observer, terminal]; let observerUsesAccess = observer == "ready" || observer == "mux_failed"; let tokens = ["none", "dictation_audio", cancel ? "none" : "dictation_audio", "none", "dictation_audio", observerUsesAccess ? "dictation_audio" : "none", cancel ? "none" : "dictation_audio", "none"]; guard object["mode"] as? String == modes[position], object["action"] as? String == actions[position], object["artifact_token"] as? String == tokens[position] else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidOrder }; if position == 7 { let result = object["result"] as? String; let equal = object["snapshot_equal"] as? Bool; guard result == "equal" && equal == true || result == "unequal" && equal == false else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidOrder }; eventAgreement.append(equal == true) } else { guard object["result"] as? String == results[position], object["snapshot_equal"] is NSNull else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidOrder } } }
        let matrixRows = try String(contentsOf: run.appendingPathComponent("matrix.csv"), encoding: .utf8).split(separator: "\n").dropFirst().map { $0.split(separator: ",", omittingEmptySubsequences: false).map(String.init) }; let matrixAgreement = matrixRows.map { $0[4] == "true" && $0[17] == "pass" }
        let environmentData = try Data(contentsOf: run.appendingPathComponent("environment.json")); guard let environment = try JSONSerialization.jsonObject(with: environmentData) as? [String: Any], let overall = environment["functional_result"] as? String else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema }
        let summary = try String(contentsOf: run.appendingPathComponent("summary.md"), encoding: .utf8); let unequalLines = summary.split(separator: "\n").map(String.init).filter { $0.hasPrefix("Unequal case:") }; let caseOrder = DevVlogsPhase0BE07CaseID.allCases.map(\.rawValue); let namedCases = unequalLines.compactMap { line in caseOrder.first { line == "Unequal case: \($0)." } }; let unequalCaseField = namedCases.isEmpty ? ["none"] : namedCases
        guard matrixRows.map({ $0[1] }) == caseOrder, eventAgreement.count == caseOrder.count, matrixAgreement == eventAgreement else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidOrder }; let failedCases = caseOrder.enumerated().compactMap { matrixAgreement[$0.offset] ? nil : $0.element }
        if overall == "pass" { guard unequalCaseField == ["none"], failedCases.isEmpty else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema } }
        else if overall == "fail" { guard !failedCases.isEmpty, namedCases == failedCases else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema } }
        else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema }
    }
