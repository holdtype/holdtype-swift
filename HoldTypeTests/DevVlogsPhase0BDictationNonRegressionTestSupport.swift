...
actor DevVlogsPhase0BE07SlowPreparationGate {
    ...
    func waitUntilEntered() async throws {
        ...
        let cancelled = Task.isCancelled
        let resolution = waitState.withLock { state -> CheckedContinuation<DevVlogsPhase0BE07GateResolution, Error>? in
            let lost = cancelled || state.cancellationRequested; state.entryWaitOpen = false; state.entryContinuation = nil
            if lost { let value = state.resolutionContinuation; state.resolutionContinuation = nil; state.resolutionWaitOpen = false; state.isClosed = true; return value }
            return nil
        }
        resolution?.resume(throwing: CancellationError()); if cancelled || waitState.withLock({ $0.cancellationRequested }) { waiterCount = 0; throw CancellationError() }
    }
    func enterAndWaitForResolution() async throws {
        ...
        let cancelled = Task.isCancelled
        waitState.withLock { state in state.resolutionWaitOpen = false; state.resolutionContinuation = nil; if !state.entryWaitOpen { state.isClosed = true } }
        if cancelled || waitState.withLock({ $0.cancellationRequested }) { waiterCount = 0; throw CancellationError() }
    }
...
@MainActor final class DevVlogsPhase0BE07Observer {
    ...
    func finish(completedArtifact: AudioRecordingArtifact, temporaryAccessToken: DevVlogsPhase0BE07TemporaryAccessToken) throws -> DevVlogsPhase0BE07ObserverCallResult { if let terminal { duplicateCount += 1; return .rejectedLateDuplicate(terminal) }; phases.append(.finalizing); let access = try temporaryAccessToken.acquire(completedArtifact: completedArtifact); defer { try? temporaryAccessToken.release(access); tokenSnapshot = temporaryAccessToken.snapshot() }; return complete(caseID == .mux_failure ? .muxFailed : .ready) }
...
struct DevVlogsPhase0BE07EvidenceValidator {
    ...
    private func validateMarkdown(_ run: URL) throws {
        let specifications: [(String, [String], [String])] = [
            ("summary.md", ["# DV-P0B-E07-W01 Summary", "## Authority", "## Scope", "## Paired Matrix Result", "## Checks", "## Redaction", "## Residuals"], ["Execution was fake-backed only.", "Exactly ten baseline/spike pairs were exercised.", "Canonical dictation retained exactly one recorder and one microphone owner.", "No second microphone was introduced.", "No product, Debug, script, project, or existing-test file changed.", "W02 and W07-R3 are provenance only and do not prove E07.", "A shipping audio lease was not exercised or proven.", "Functional result: pass."]),
    ...
    private func validateEnvironment(_ run: URL) throws {
        ...
        guard Set(object.keys) == Set(keys), object["schema_version"] as? String == "dv-p0b-e07-environment-v1", object["packet_id"] as? String == "DV-P0B-E07-W01", object["contract_revision"] as? String == "DV-DRAFT-4@2f3266a", object["implementation_parent_commit"] as? String == "24f4a601087322c6a3ac80e637e808a684deb9b4", object["execution_mode"] as? String == "fake_backed", object["functional_result"] as? String == "pass" else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema }
        ...
        let text = String(decoding: data, as: UTF8.self); for key in keys where text.components(separatedBy: "\"\(key)\"").count != 2 { throw DevVlogsPhase0BE07EvidenceValidationError.duplicateKey }
    }
    private func validateCSVFiles(_ run: URL) throws {
        ...
        let measurementRows = try String(contentsOf: run.appendingPathComponent("measurements.csv"), encoding: .utf8).split(separator: "\n").dropFirst().map(String.init); guard measurementRows.first == "dv-p0b-e07-measurements-v1,camera_slow_timeout,recording_before_gate_resolution,true,boolean,functional_gate", measurementRows.dropFirst().allSatisfy({ $0.hasPrefix("dv-p0b-e07-measurements-v1,all,") && $0.hasSuffix(",not_measured,not_applicable,not_applicable_fake_backed") }) else { throw DevVlogsPhase0BE07EvidenceValidationError.invalidSchema }
