...
struct OpenAITranscriptionRequestBuilderTests {
    ...
    @Test func includesActiveTextContextWhenEnabled() throws {
        ...
        let context = try #require(
            TranscriptionPromptContext("We are writing about HoldType settings.")
        )
        ...
        #expect(
            bodyText.contains(
                ...
                We are writing about HoldType settings.
                """
            )
        )
    }
    ...
    @Test func omitsActiveTextContextWhenDisabled() throws {
        ...
        #expect(bodyText.contains("We are writing about HoldType settings.") == false)
    }

    @Test func includesEmojiCommandHintsWhenEnabled() throws {
        let audioFileURL = try makeTemporaryAudioFile(
            named: "recording.m4a",
            contents: Data("fake audio bytes".utf8)
        )
        ...
        #expect(bodyText.contains("Content-Disposition: form-data; name=\"prompt\""))
        #expect(bodyText.contains("Emoji command vocabulary"))
        #expect(bodyText.contains("эмодзи улыбка"))
        #expect(bodyText.contains("эмодзи смех"))
        #expect(bodyText.contains("эмодзи сердце"))
        #expect(bodyText.contains("эмоции") == false)
        #expect(bodyText.contains("эмоджи") == false)
        #expect(bodyText.contains("emoji smile") == false)
        #expect(bodyText.contains("emoji fire") == false)
    }
    ...
    private func makeTemporaryAudioFile(named fileName: String, contents: Data) throws -> URL {
        let directoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("holdtype-request-builder-\(UUID().uuidString)", isDirectory: true)
