...
struct TranscriptHistoryEntryTests {
    ...
    @Test func createsEntryFromAcceptedTranscriptMetadata() throws {
        ...
        let entry = try TranscriptHistoryEntry(
            ...
            languageCode: " en ",
            audioDuration: 2.5,
            cachedAudioFileURL: URL(fileURLWithPath: "/tmp/HoldType-20260707-120000.m4a")
        )
        ...
        #expect(entry.cachedAudioFileURL?.path == "/tmp/HoldType-20260707-120000.m4a")
    }
    ...
    @Test func encodesAndDecodesOnlyAllowedHistoryFields() throws {
        ...
        #expect(encodedKeys.contains("cachedAudioFileURL") == false)
    ...
    @Test func cachedAudioFileURLIsSessionOnlyAndNotCodable() throws {
        let entry = try TranscriptHistoryEntry(
            transcriptText: "Accepted transcript",
            transcriptionModel: "gpt-4o-transcribe",
            languageCode: "en",
            cachedAudioFileURL: URL(fileURLWithPath: "/tmp/HoldType-cache.m4a")
        )

        let data = try JSONEncoder().encode(entry)
        let decoded = try JSONDecoder().decode(TranscriptHistoryEntry.self, from: data)

        #expect(entry.cachedAudioFileURL?.path == "/tmp/HoldType-cache.m4a")
        #expect(decoded.cachedAudioFileURL == nil)
    }
