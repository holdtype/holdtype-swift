...
This directory is the executable development queue for HoldType Swift.
...
Use full `--json` only when compact output lacks diagnostics needed to debug
the queue.
...
`python3 scripts/backlog_next.py --compact-json`, run `git diff --check`, and
create a scoped checkpoint commit for the moved task files.
