...
This file is the root-level development contract for explicit backlog work,
scheduled backlog automations, backlog grooming, and user-requested restartable
agent runs.
...
Backlog-mode development is coordinated through committed backlog tasks, not a
live chat thread.

A chat may still discuss architecture, create backlog tasks, review results, or
decide priorities. In backlog mode, the durable source of truth for work state
...
- explicit user requests to use, create, select, decompose, groom, archive, or
  execute backlog tasks;
- scheduled Codex automations whose prompt or runbook says they are backlog
  workers;
- backlog files, backlog scripts, backlog runbooks, and backlog maintenance;
- manual Codex iterations that the user explicitly wants to make restartable
  through committed backlog tasks;
- long migrations where the user and agent explicitly agree that durable task
  state and executable follow-up work should live in backlog files.

Do not use backlog as the default planning mechanism for ordinary direct chat
work. If a direct chat task needs coordination or future follow-up, describe it
in the chat first. Create backlog files only after explicit user approval or
when the active automation/runbook requires them.
...
This rule applies only in backlog mode.
...
1. Run `python3 scripts/backlog_next.py --compact-json` from the canonical
   checkout. This standard selector run expires stale `in-progress` claims by
   default while keeping agent context small. Use full `--json` only when
   compact output lacks diagnostics needed for the current decision.
2. If `expired_in_progress_reset_paths` is non-empty, run `git diff --check`,
   ...
   commit such as `Expire stale backlog claims`, and rerun the same compact
   selector command before claiming work.
...
processes and remove generated HoldType artifacts such as project-scoped
